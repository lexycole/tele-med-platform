...
// import listkanbans from "../../api/listkanbans";
// import tasksApi, { saveTask } from "../../api/tasks";
...
const validationSchema = Yup.object().shape({
  ...
  taskNo: Yup.string().optional("Optional"),
...
const OperationsTask = ({ route, navigation }) => {
  const makeTaskNo = () => {
    let taskNumber = "TA-";
    const possible = "ABCDEFGHIJKLMNPQRSTUVWXYZ2356789";
    for (let i = 0; i <= 6; i++)
      taskNumber += possible.charAt(
  ...
  useEffect(() => {
    if (!_.isEmpty(selectedTask)) {
      setInitialState({
        ...selectedTask,
        participants: selectedTask.participants.map((item) => item._id),
        user: user?._id,
        taskNo: makeTaskNo(),
      });
    }
  ...
  const handleSubmit = async (userInfo) => {
    ...
    try {
      const { ok, data } = await saveTask(submitData);
      console.log(ok)
      setLoading(false);
      if (!ok) {
      ...
      alert(submitData._id ? "Task Updated" : "Task added");
      ...
      <View style={styles.Container}>
        ...
        <KeyboardAwareScrollView>
          ...
          <Navbar
            ...
            Text={!_.isEmpty(selectedTask) ? "Edit Task" : "Add Task"}
          />
          {/* <ActivityIndicator
            visible={
              loading
                ...
                <>
                  ...
                  >
                    ...
                    <View style={{ width: "80%", marginTop: 20 }}>
                      <AppMultiDropdown
                        title="ListKanbans"
                        placeholder="Select ListKanban"
                        options={getListKanbans()}
                      />
                    </View>
