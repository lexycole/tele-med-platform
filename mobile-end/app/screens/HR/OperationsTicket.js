...
// import ticketsApi, { saveTicket } from "../../api/tickets";
...
const validationSchema = Yup.object().shape({
  ...
  narrative: Yup.string().optional("Optional"),
  ...
  subDepartment: Yup.string().optional("Optional"),
...
const OperationsTicket = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if(!_.isEmpty(selectedTicket)) {
      setInitialState({...selectedTicket, participants: selectedTicket.participants.map(item => item._id)});
    }
  },[selectedTicket])
                ...
                >
                  ...
                  >
                    <Title style={{ fontWeight: "bold", marginLeft: 8 }}>
                     {!_.isEmpty(selectedTicket) ? selectedTicket.name: 'Add Ticket'}  
                    </Title>
                  </View>
                  ...
                  <View style={{ width: "82%", marginTop: 15 }}>
                    <Text style={{ marginBottom: 5, fontSize: 16, fontWeight: "700" }}>Deadline:</Text>
