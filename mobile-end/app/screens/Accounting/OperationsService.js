...
const Operationsservice = ({ route, navigation }) => {
  const [selected, setSelected] = useState({});

  const serviceApi = useApi(servicesApi.saveservice);
  const getCountriesApi = useApi(countries.getCountries);
  const [error, setError] = useState();

  useEffect(() => {
    if (route.params) {
      setSelected(route.params.selectedservice);
    }
  ...
  const handleSubmit = async (userInfo) => {
    ...
    try {
      ...
      const result = await serviceApi.request(submitData);
      ...
      <View style={styles.Container}>
        ...
        <KeyboardAwareScrollView>
          ...
          <Navbar
            ...
            Text={`${_id ? "Update" : "Add"} service`}
          />

          <ActivityIndicator
            visible={serviceApi.loading || getCountriesApi.loading}
          />
