...
// import topicsApi, { saveTopic } from "../../api/topics";
...
const OperationsTopic = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(selectedTopic)) {
      setInitialState({
        ...selectedTopic,
        participants: selectedTopic.participants.map((item) => item._id),
        user: user?._id,
        topicNo: makeTopicNo(),
      });
    }
  ...
  const handleSubmit = async (userInfo) => {
    ...
    try {
      const { ok, data } = await saveTopic(submitData);
      console.log(ok)
      setLoading(false);
      if (!ok) {
      ...
      alert(submitData._id ? "Topic Updated" : "Topic added");
      ...
      <View style={styles.Container}>
        ...
        <KeyboardAwareScrollView>
          ...
          <Navbar
            ...
            Text={!_.isEmpty(selectedTopic) ? "Edit Topic" : "Add Topic"}
          />
          <ActivityIndicator
            visible={
              loading
