...
const RemedyFields = ({ dataRemedy, dataPotency, values, name, setFieldlValue, isEdit }) => {
  const { isTablet } = useSnapshot(state);

  const addHandler = () => {
    setFieldlValue(name, [
      ...values[name],
      { materiaMedica: '', potency: ""},
    ]);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.buttonTitle}>Add Remedy to Treatment</Text>
      </TouchableOpacity>
        ...
        {values[name].map((input, key) => (
          <View style={{
            ...
            <View style={{ flexDirection: "row", width: isTablet ? "33%" : "100%", justifyContent: "space-between", alignItems: "center" }}>
              ...
              >
                <Text style={{ marginBottom: 5 }}>Potency :</Text>
                <View
                  style={[
                    {
                      height: 58,
                ...
                >
                  <View style={{ flex: 1 }}>
                    <TextInput
                      ...
                      value={input["dosage"]}
                      onChangeText={value => updatePotency(value, key)}
                      placeholder="Potency"
