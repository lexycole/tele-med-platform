 ...
 */
 ...
 // Bear all forum date
 import { categoryDate, Tags } from "../screens/Forum/categorydata";
...
const tags =["red","pink","yellow","lightblue","grey","orange","#6C3483","#76D7C4","#196F3D"]
 ...
 const CategoryIndex = ({navigation,route}) => {
...
useEffect( ()=>{
  getTopicsOfCategory();
},[route?.params?.categoryData?._id]);
//},[]);
 
const getTopicsOfCategory = async()=>{
  const {data:topics} = await getTopics();
  //console.log(topics.length)
  const filteredTopics = topics.filter( e=> e.catId._id===route?.params?.categoryData?._id);
      ...
      <SafeAreaView style={{flex:1}}>
       <NavForum
          onPress1={() => {
            ...
            {
              catId : route?.params?.categoryData._id
            }
            );
          }}
          onPress={() => {
           ...
           >
             <View style={styles.naming}>
               <Text style={styles.namingText}>Topics</Text>
                 ...
                 <View >
                  {route?.params?.categoryData && <Category arr={route?.params?.categoryData} />}
                   {!landscape && 
                   <View>
                    {
                      topics.length>0 && topics?.map((Element,index)=>{ 
                        return (
                          <Latest key={index} data={Element} navigation={navigation}  />
  ...
  return (
    ...
    <View style={{ width: "100%" }}>
      <View style={styles.person_info}>
        ...
        <Text className="username">{data?.user?.contactName.first+" "+data?.user?.contactName.last}</Text>
        </View>
